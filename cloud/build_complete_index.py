...
class CompleteIndexBuilder:
    ...
    def __init__(self):
        ...
        # Setup ChromaDB
        print("[BUILDER] Setting up ChromaDB client...", flush=True)
        ...
        print(f"[BUILDER] ChromaDB client created at {chroma_path}.", flush=True)
        
        print("[BUILDER] Loading embedding function (all-MiniLM-L6-v2)...", flush=True)
        self.embedding_function = embedding_functions.SentenceTransformerEmbeddingFunction(
            model_name="all-MiniLM-L6-v2"
        )
        print("[BUILDER] Embedding function loaded.", flush=True)
    ...
    def load_docs_data(self):
        ...
        print(f"Loaded {len(docs)} documentation pages", flush=True)
        return docs
    ...
    def load_website_data(self):
        ...
        print(f"Loaded {len(website)} website pages", flush=True)
        return website
    ...
    def load_blog_data(self):
        ...
        print(f"Loaded {len(blog)} blog posts", flush=True)
        return blog
    ...
    def prepare_all_chunks(self):
        ...
        print("\nChunking documentation...", flush=True)
        for doc in docs:
        ...
        # Chunk website data
        print("\nChunking website pages...", flush=True)
        ...
        # Chunk blog posts
        print("\nChunking blog posts...", flush=True)
    ...
    def build_index(self):
        ...
        try:
            ...
            print("\nDeleted existing 'auto_finance_complete' collection", flush=True)
        except:
            pass
        
        # Create new collection
        print("\nCreating new collection: 'auto_finance_complete'", flush=True)
        ...
        for i in range(0, len(all_chunks), batch_size):
            ...
            print(f"  Batch {i//batch_size + 1}/{(len(all_chunks)-1)//batch_size + 1}", flush=True)
        
        print(f"\n[OK] Index built successfully!", flush=True)
        print(f"  Total chunks indexed: {len(all_chunks)}", flush=True)
        print(f"  Collection: 'auto_finance_complete'", flush=True)
        print(f"  Original 'auto_finance_docs' collection preserved", flush=True)
    ...
    def verify_index(self):
        ...
        try:
            ...
            print(f"\n[OK] Verification: Collection has {count} chunks", flush=True)
            ...
            print(f"[OK] Test search successful, found {len(results['ids'][0])} results", flush=True)
            return True
